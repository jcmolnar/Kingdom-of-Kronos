...
//
//-----------------------------------
// Clear ALL old serverTempData files BEFORE loading prefs
// This prevents any auto-loading issues when $Server::Port is set
echo("===== CLEARING serverTempData FILES (console.cs) =====");

// Clear ALL old serverTempData files BEFORE loading prefs
// This prevents any auto-loading issues when $Server::Port is set
echo("===== CLEARING serverTempData FILES (console.cs) =====");
echo("Searching for pattern: temp\\serverTempData*.cs");

// First, try to find all files with wildcard
%filePattern = "temp\\serverTempData*.cs";
%file = File::findFirst(%filePattern);
echo("File::findFirst with wildcard returned: " @ %file);
...
{
	%fileCount++;
	echo("Found serverTempData file #" @ %fileCount @ ": " @ %file);
	
	if(isFile(%file))
	{
		echo("  File exists before deletion: YES");
		File::delete(%file);
		
		if(isFile(%file))
			echo("  ERROR: File still exists after deletion: " @ %file);
		else
			echo("  Success: File deleted: " @ %file);
		%deletedCount++;
	}
	
	%file = File::findNext(%filePattern);
}
...
//newObject(ScriptsVolume, SimVolume, "scripts.vol.backup");
