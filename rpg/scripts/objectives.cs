...
function ObjectiveMission::initCheck(%object)
{
   ...
   // Store original position for map flags (team -1, not house flags) so they can reset after being dropped
   %name = gamebase::getdataname(%object);
   if(%name == "flag" || %name == "Flag")
   {
      %flagTeam = GameBase::getTeam(%object);
      // Map flags are team -1 and don't have holdingTeam property (house flags have holdingTeam)
      if(%flagTeam == -1 && (%object.holdingTeam == "" || %object.holdingTeam == -1))
      {
         // Store the original position from the mission file as the default reset position
         if(%object.originalPosition == "")
...
function TowerSwitch::onCollision(%this, %object)
{
   ...
   if(%this.team != "")
   	$BaseControl[%this.team] -= 1;
   $BaseControl[%playerTeam] += 1;
   UpdateHouseObjectivesDisplay();

   %this.trainingObjectiveComplete = true;
   %this.team = %playerTeam;  // Store house name in .team property
   
  
   %touchClientName = Client::getName(%playerClient);
   %group = GetGroup(%this);
   // Set all objects in tower group to team 0 (all players are team 0)
   // But turrets should stay on team 1 (enemy of team 0 players) to activate targeting
   %objCount = Group::objectCount(%group);
   for(%i = 0; %i < %objCount; %i++)
   {
       %obj = Group::getObject(%group, %i);
       %objDataName = GameBase::getDataName(%obj);
       // Turrets should stay on team 1 (enemy of team 0 players) to activate targeting
       if(String::findSubStr(%objDataName, "Turret") == -1)
       {
   ...
   for(%i = 0; %i < %objCount; %i++)
   {
       ...
       if(getObjectType(%obj) == "SimGroup")
       {
           ...
           for(%j = 0; %j < %subObjCount; %j++)
           {
               ...
               %subObjDataName = GameBase::getDataName(%subObj);
               // Turrets should stay on team 1 (enemy of team 0 players) to activate targeting
               if(String::findSubStr(%subObjDataName, "Turret") == -1)
               {
                   GameBase::setTeam(%subObj, 0);
               }
               else
               {
                   // Ensure turrets stay on team 1 for player targeting (team -1 doesn't activate targeting)
   ...
   Group::iterateRecursive(%group, TurretTeam, %playerTeam);
...
function Flag::onDrop(%player, %type)
{
   ...
   if(%flagTeam == -1)
   {
      ...
      // Check if this is a map flag that was being held by a house
      // Use previousHoldingTeam (stored when flag was picked up) since holdingTeam was cleared
      %holdingHouse = %flag.previousHoldingTeam;
      if(%holdingHouse == "" || %holdingHouse == -1)
         %holdingHouse = %flag.holdingTeam; // Fallback to current holdingTeam if previousHoldingTeam not set
      
      if(%holdingHouse != "" && %holdingHouse != -1 && %holdingHouse != "None")
      {
         // Format house name for display (remove "House" prefix)
         %houseDisplayName = %holdingHouse;
         if(String::findSubStr(%houseDisplayName, "House") == 0)
            %houseDisplayName = String::getSubStr(%houseDisplayName, 5, 99999); // Remove "House" prefix
         
         // Global broadcast message for map flag drop from house
         MessageAll(0, %dropClientName @ " has attempted to steal the Artifact from " @ %houseDisplayName @ " but dropped it - The Artifact will reset to its original position in 1 minute! Reclaim it before it is too late!");
      }
      
      // Clear previousHoldingTeam after using it
      %flag.previousHoldingTeam = "";
   }
   else
   {
   ...
   // Map flags (team -1) reset after 1 minute (60 seconds)
   // House flags and team flags use the default $flagReturnTime (45 seconds)
   %resetTime = $flagReturnTime;
   if(%flagTeam == -1)
   {
      // This is a map flag (not a house flag), reset after 1 minute
      %resetTime = 60;
   }
   
   schedule("Flag::checkReturn(" @ %flag @ ", " @ %flag.pickupSequence @ ");", %resetTime);
	%flag.dropFade = 1;
   ObjectiveMission::ObjectiveChanged(%flag);
   
   // Update objectives display and save house objectives when flag is dropped
   UpdateHouseObjectivesDisplay();
   SaveHouseObjectives();
}
		...
		if(%flag.dropFade) { 
         ...
         %flag.atHome = true;
   	   ...
   	   %flag.fadeOut= "";
			...
			// Update objectives display and save house objectives when flag resets
			UpdateHouseObjectivesDisplay();
			SaveHouseObjectives();
		}
   }
   ...
   else
   {
      ...
      if(%object.carryFlag == "")
      {
			if(%object.outArea == "") {
        		...
        		//{
	     		   ...
	     		   if(%hteam != -1)
        		   {
        		      ...
        		      $deltaTeamScore[%hteam] -= %this.deltaTeamScore;
	     		      ...
	     		      MessageAllExcept(%playerClient, 0, %touchClientName @ " took " @ %this.objectiveName @ " from " @ %flagTeam @ ".~wflag1.wav");
        		      Client::sendMessage(%playerClient, 0, "You took " @ %this.objectiveName @ " from " @ %flagTeam @ ".~wflag1.wav");
        		      ...
        		      // Store the house name before clearing holdingTeam, so we can show it in drop message
        		      %this.previousHoldingTeam = %hteam;
        		      %this.holdingTeam = -1;
        		      %this.holder.flag = "";
			      $FlagCommand[%flagTeam] -= 1;
			      UpdateHouseObjectivesDisplay();
			      SaveHouseObjectives();
        		   }
        		   else
        		...
        		// Reset GameBase::setTeam to -1 when flag is picked up so it can be captured at any flagstand
...
function FlagStand::onCollision(%this, %object)
{
   ...
   %flag = %object.carryFlag;
   %flag.carrier = -1;
   %flag.team = %playerTeam;  // Store house name
   Item::hide(%flag, false);
   ...
   %flag.holder = %this;
   %flag.holdingTeam = %playerTeam;  // Store house name
   %this.flag = %flag;

   $FlagCommand[%playerTeam] += 1;
   UpdateHouseObjectivesDisplay();
   SaveHouseObjectives();
   MessageAllExcept(%playerClient, 0, Client::getName(%playerClient) @ " conveyed " @ %flag.objectiveName @ " to " @ %playerTeam @ ".");
...
{
   //echo(%this @ " " @ %clientId);
   // When player disconnects, drop any flag they're carrying
   // Get the player object from client ID
   %player = Client::getOwnedObject(%clientId);
   if(%player != -1 && %player != "" && %player.carryFlag != "" && %player.carryFlag != -1)
   {
      // Player is carrying a flag - drop it using Flag::onDrop to trigger all the proper logic
      Flag::onDrop(%player, "");
   }
   else
   {
      // Fallback: try to drop mounted flag item
