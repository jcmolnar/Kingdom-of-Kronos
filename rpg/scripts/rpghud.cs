...
function initRPGHUD()
{
	// Only initialize HUD on client side (not on dedicated servers)
	// HUD functions require PrestoPack and are client-side only
	if($dedicated)
		return;
...
// Initialize HUD after a short delay to ensure PrestoPack is loaded
// Only call on client side (we're already past the $dedicated check)
if(!$dedicated)
	schedule("initRPGHUD();", 0);
