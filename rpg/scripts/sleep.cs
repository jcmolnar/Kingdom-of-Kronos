function GroupTrigger::onTrigEnter(%object, %this)
{
	...
	else if(String::ICompare(Object::getName(getGroup(getGroup(getGroup(%object)))), "TeleportBoxes") == 0)
	{
		...
		if(%h != 667 && %h != 666 && %h != False)
		{
			...
			// Use centerprint for teleport success message
			// Extract sound from message if present (~wsound.wav format)
			%soundPos = String::findSubStr(%gsay, "~w");
			%textMsg = %gsay;
			%soundName = "";
			if(%soundPos != -1)
			{
				%textMsg = String::getSubStr(%gsay, 0, %soundPos);
				%soundWithExt = String::getSubStr(%gsay, %soundPos + 2, 99999);
				// Remove .wav extension if present to get sound name
				%dotPos = String::findSubStr(%soundWithExt, ".");
				if(%dotPos != -1)
					%soundName = String::getSubStr(%soundWithExt, 0, %dotPos);
				else
					%soundName = %soundWithExt;
			}
			
			%gsayLen = String::len(%textMsg);
			%timeout = floor(%gsayLen / 20);
			
			// Play sound separately if present
			// Delay ensures sound plays at destination, not origin
			if(%soundName != "")
			{
				// Convert lowercase sound name to proper case (portal6 -> Portal6)
				%firstChar = String::getSubStr(%soundName, 0, 1);
				%restOfName = String::getSubStr(%soundName, 1, 99999);
				// Simple capitalization: a->A, b->B, etc.
				%upperChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
				%lowerChars = "abcdefghijklmnopqrstuvwxyz";
				%lowerPos = String::findSubStr(%lowerChars, %firstChar);
				if(%lowerPos != -1)
					%firstCharUpper = String::getSubStr(%upperChars, %lowerPos, 1);
				else
					%firstCharUpper = %firstChar;
				%properSoundName = %firstCharUpper @ %restOfName;
				// Get position when sound actually plays (at destination after teleport)
				schedule("if(Client::getName(" @ %clientId @ ") != \"\") { %pos = GameBase::getPosition(" @ %clientId @ "); playSound(" @ %properSoundName @ ", %pos); }", 0.22);
			}
			
			// Display message in centerprint (without sound syntax)
			schedule("if(Client::getName(" @ %clientId @ ") != \"\") centerprint(" @ %clientId @ ", \"<jc>\" @ \"" @ %textMsg @ "\", " @ %timeout @ ");", 0.22);
		}
		else
		{
			// Use centerprint for teleport warning message
			%msg = "<jc>" @ %nsay;
			if(Client::getName(%clientId) != "")
				centerprint(%clientId, %msg, floor(String::len(%nsay) / 20));
		}
	}
}
