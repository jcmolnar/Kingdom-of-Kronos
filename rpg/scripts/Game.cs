...
$TeamEnergy[8]  = $DefaultTeamEnergy;
$TeamEnergy[9]  = $DefaultTeamEnergy;
$TeamEnergy[10] = $DefaultTeamEnergy;
$TeamEnergy[11] = $DefaultTeamEnergy;
$TeamEnergy[12] = $DefaultTeamEnergy; 
...
function Game::notifyMatchStart(%time)
{
	...
	// Start persistent double-execution warning
	if($ServerPrefs::DoubleExecuted)
	{
		schedule("spamDoubleExecWarning();", 0.5);
	}
}

function spamDoubleExecWarning()
{
	if($ServerPrefs::DoubleExecuted)
	{
		echo("===================================================");
		echo("WARNING: ServerPrefs.cs was executed TWICE! THIS BREAKS EVERYTHING ON RESTART!");
		echo("Check your InfiniteSpawn.exe shortcut properties TARGET field for +exec ServerPrefs.cs - Console.cs already executes it for you!");
		echo("===================================================");
		schedule("spamDoubleExecWarning();", 0.5);
	}
}
...
// Stub functions to prevent temp file errors
function Game::onPlayerConnected(%game, %clientId)
{
	// Called when player connects - handled elsewhere
	return;
}

function messageAll(%msgType, %message)
{
	// Send message to all clients
	for(%cl = Client::getFirst(); %cl != -1; %cl = Client::getNext(%cl))
	{
		Client::sendMessage(%cl, %msgType, %message);
	}
}
