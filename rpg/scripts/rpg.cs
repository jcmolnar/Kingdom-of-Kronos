//============================================================================	
//Handle all linebreaks before they can become a problem
//This way I can cut down on .cs files
//============================================================================
...
function LineBreak::Test(%client, %cropped)
{
    ...
    for(%i = 1; %i < 256; %i++)
    {
        if(%i < 32 || %i > 126)
        {
            ...
            {
                $linebreaker = "Name: "@client::getname(%client) @ " Linebreak: "@%cropped@" | HEX: " @ %h @ " DEC: " @ %i;
                echo("===[LineBreak::Check detected invalid characters: " @ $linebreaker@ "]");
                export("linebreaker", "temp\\[Rpg-Linebreak].cs", true);
...
$hex["30"] = "\x30"; $hex["31"] = "\x31"; $hex["32"] = "\XMarkTwo"; $hex["33"] = "\x33";
