...
$SkillMultiplier[Cleric, $SkillVehicleCombat] = 1.0;
...
$SkillMultiplier[Druid, $SkillVehicleCombat] = 1.0;
...
//Secondary Skill: Hiding, Backstabbing, Piercing, Vehicle Combat
...
$SkillMultiplier[Thief, $SkillVehicleCombat] = 1.0;
...
//Primary Skill: Stealing
//Secondary Skill: Vehicle Combat
...
$SkillMultiplier[Bard, $SkillVehicleCombat] = 1.0;
...
$SkillMultiplier[Fighter, $SkillVehicleCombat] = 1.0;
...
$SkillMultiplier[Paladin, $SkillVehicleCombat] = 1.0;
...
$SkillMultiplier[Squire, $SkillVehicleCombat] = 1.0;
...
$SkillMultiplier[Mage, $SkillVehicleCombat] = 1.0;
...
$SkillMultiplier[RedFlag, $SkillVehicleCombat] = 1.0;
...
//	(weapon):
//	a = ATK * 1.1 (vehicle combat is 0.75)
...
$SkillRestriction[RedDiamondHelmet] = $SkillEndurance @ " 1950 " @ $MinRemort @ " 50";
$SkillRestriction[WhiteDiamondHelmet] = $SkillEndurance @ " 2400 " @ $MinRemort @ " 75";
...
$SkillRestriction[GodlyAntiMagicBelt] = $SkillEndurance @ " 2700 " @ $MinRemort @ " 50";
$SkillRestriction[HeavenlyAntiMagicBelt] = $SkillEndurance @ " 4050 " @ $MinRemort @ " 75";
$SkillRestriction[GodlyPowerRing] = $MinLevel @ " 100";
$SkillRestriction[HeavenlyPowerRing] = $MinRemort @ " 75";
$SkillRestriction[GodlyRegenerationNecklace] = $MinRemort @ " 50";
$SkillRestriction[HeavenlyRegenerationNecklace] = $MinRemort @ " 75";
...
$SkillRestriction[RedDiamondKingShield] = $SkillEndurance @ " 3900 " @ $MinRemort @ " 50";
$SkillRestriction[WhiteDiamondKingShield] = $SkillEndurance @ " 4800 " @ $MinRemort @ " 75";
...
$SkillRestriction[advshield6] = $SkillDefensiveCasting @ " 850";
...
function GetNumSkills()
{
	...
	// Initialize %i at the start, then use it in the for loop
	%i = 1;
	for(%i = 1; $SkillDesc[%i] != ""; %i++)
	{
		// Loop through skills to count them
	}
	return %i-1;
}
...
function AddSkillPoint(%clientId, %skill, %delta)
{
	...
	////////temporary/////////////////////////
	if(%skill == 7)	//stealing (no longer used)
		return False;
	if(%skill == 17)	//mining (no longer used)
		return False;

	if(%skill == 6)	//bashing
	{
		// Check BEFORE increasing to prevent going over 1000
		if($PlayerSkill[%clientId, $SkillBashing] >= 1000)
		{
			$PlayerSkill[%clientId, $SkillBashing] = 1000;
			return false;
		}
	}
	if(%skill == 14)	//vehicle combat
	{
		if($PlayerSkill[%clientId, $SkillVehicleCombat] >= 1000)
		{
			$PlayerSkill[%clientId, $SkillVehicleCombat] = 1000;
			return false;
		}
	}
	...
	if(%skill == 18)	//speech
	{
		if($PlayerSkill[%clientId, $SkillSpeech] >= 100)
		{
			$PlayerSkill[%clientId, $SkillSpeech] = 100;
			return false;
		}
	}
	if(%skill == 21)	//haggle
	...
	if(%skill == 16)
	{
		...
		// Initialize %a with default value if class or multiplier doesn't exist
		%a = 0.5;  // Default critical skill multiplier
		if(%class != "" && %class != -1)
		{
			if($SkillMultiplier[%class, $SkillCriticals] != "")
				%a = $SkillMultiplier[%class, $SkillCriticals];
		}
		
		%b = $PlayerSkill[%clientId, %skill];
		%c = %a + %b;
		// Critical skill: round to nearest 0.1
		%d = round(%c * 10);
		%e = %d / 10;

		$PlayerSkill[%clientId, %skill] = %e;
		
		// Cap skills after increase to prevent exceeding limits
		if(%skill == 6 && $PlayerSkill[%clientId, $SkillBashing] > 1000)
		{
			$PlayerSkill[%clientId, $SkillBashing] = 1000;
		}
		if(%skill == 14 && $PlayerSkill[%clientId, $SkillVehicleCombat] > 1000)
		{
			$PlayerSkill[%clientId, $SkillVehicleCombat] = 1000;
		}
		if(%skill == 16 && $PlayerSkill[%clientId, $SkillCriticals] > 1000)
		{
			$PlayerSkill[%clientId, $SkillCriticals] = 1000;
		}
		if(%skill == 18 && $PlayerSkill[%clientId, $SkillSpeech] > 100)
		{
			$PlayerSkill[%clientId, $SkillSpeech] = 100;
		}
		if(%skill == 21 && $PlayerSkill[%clientId, $SkillHaggling] > 1000)
		{
			$PlayerSkill[%clientId, $SkillHaggling] = 1000;
		}
	...
	else
	{
		...
		// Use FixDecimals to preserve fractional skill gains (important for multipliers below 1.0)
		// This allows small gains (e.g., 0.1, 0.2) to accumulate over time instead of being lost to rounding
		if(%skill == $SkillCriticals)
		{
			// Critical skill: round to nearest 0.1
			%d = round(%c * 10);
			%e = %d / 10;
		...
		{
			// All other skills: use FixDecimals to preserve 1 decimal place precision
			// This ensures multipliers below 1.0 can still increase skills over time
			%e = FixDecimals(%c);
		}
...
// Format skill value for display: rounds up, no decimals (except Critical skill which can show 1 decimal)
function FormatSkillDisplay(%clientId, %skill)
{
	%value = GetPlayerSkill(%clientId, %skill);
	...
	{
		// Has decimal - format to show exactly 1 decimal place
		%roundedStr = %rounded;
		%decimalPos = String::findSubStr(%roundedStr, ".");
		// Truncate to 1 decimal place
		%roundedStr = String::getSubStr(%roundedStr, 0, %decimalPos + 2);
		return %roundedStr;
	}
}
function GetSkillMultiplier(%clientId, %skill)
{
	...
	// Initialize %a with default value if class or multiplier doesn't exist
	%a = 1.0;  // Default multiplier
	if(%class != "" && %class != -1)
	{
		if($SkillMultiplier[%class, %skill] != "")
			%a = $SkillMultiplier[%class, %skill];
	}
...
function SetAllSkills(%clientId, %n)
{
	...
	// Initialize %n if not provided to prevent debug warnings
	if(%n == "" || %n == -1)
		%n = 0;
	
	// Get the number of skills safely
	%numSkills = GetNumSkills();
	for(%i = 1; %i <= %numSkills; %i++)
	{
		// Use temp variable to safely check if SkillDesc exists before accessing
		%tempDesc = $SkillDesc[%i];
		if(%tempDesc != "")
...
function SkillCanUse(%clientId, %thing)
{
	...
	%gc = 0;
	%gcflag = 0;
	%hflag = 0;  // Initialize house flag counter
	%hh = 0;     // Initialize house match flag
	
	// Use temp variable to safely check if SkillRestriction exists before accessing
	%tempRestriction = $SkillRestriction[%thing];
	if(%tempRestriction == "")
		%tempRestriction = ""; // Ensure it's initialized (already is, but explicit)
	
	for(%i = 0; GetWord(%tempRestriction, %i) != -1; %i+=2)
	{
		%s = GetWord(%tempRestriction, %i);
		%n = GetWord(%tempRestriction, %i+1);
...
function UseSkill(%clientId, %skilltype, %successful, %showmsg, %base, %refreshall)
{
	...
	if(%base == "") %base = 30;
	if(%refreshall == "") %refreshall = false;  // Initialize refreshall if not provided
