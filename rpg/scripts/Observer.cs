...
function Observer::enterObserverMode(%clientId)
{
        ...
        %clientId.observerMode = "observerOrbit";
        // CRITICAL: Don't reset team for bots - they should never be in observer mode
        // This is a safeguard in case a bot somehow reaches this code path
        if(!Player::isAiControlled(%clientId) && !isRPGAI(%clientId) && !IsBotRegistered(%clientId))
