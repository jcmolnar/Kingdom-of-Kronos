...
// Kingdom of Kronos is a client-wide RPG extension as well as a selectable
// ModernHUD provider. Load it once from the shared framework so its remote
// receivers, menu/shop/chat overlays and slot borrowers remain available when
// the player selects a different primary pack.
if($KronosHUD::ClientLoaded == "" && isFile("Presto/KronosHUD.cs"))
   exec("Presto/KronosHUD.cs");

// ModernHUD::unload clears the settings registry at every pack swap. Re-add the
// cross-pack RPG rows before the newly selected pack appends its own settings.
if(isFunction("KronosHUD::registerSettings"))
   KronosHUD::registerSettings();
