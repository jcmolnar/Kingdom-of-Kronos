...
function Vector::menu(%screen)
{
   ...
   // -- body ------------------------------------------------------------------
   // Solid base first (opaque), then the theme tint over it.
   Vector::color("10 13 16", 252);
   glRectangle(%x, %y, %w, %h);
   Vector::color($Vector::Dim, 90);
   glRectangle(%x, %y, %w, %h);

   // Edge light: accent spine down the left, hairlines elsewhere.
   Vector::color($Vector::Primary, 255);
   glRectangle(%x, %y, 3, %h);
   ...
   // Chamfered top-right corner -- the pack's own shape language.
   Vector::color("10 13 16", 252);
   glAngledPolygon(%x + %w - 18, %y, %x + %w, %y, %x + %w, %y + 18, %x + %w - 18, %y);
   Vector::color($Vector::Primary, 200);
   glAngledPolygon(%x + %w - 18, %y, %x + %w, %y + 18, %x + %w - 1, %y + 18, %x + %w - 17, %y);
   ...
   %i = Vector::menuItem(%x, %ry, %w, %i, %mx, %my, "Colour theme",
        Vector::themeName($pref::Vector::Theme), "pref::Vector::Theme", 1, 0, 3);
   %ry = %ry + 24;
