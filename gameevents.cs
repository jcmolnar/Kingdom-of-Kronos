...
function HouseEarnings()
{
	...
	for(%cl = Client::getFirst(); %cl != -1; %cl = Client::getNext(%cl))
	{
		...
		if(%house != "")
		{
			%reward = ((10000 * (fetchData(%cl, "RemortStep") + 1)) + (1000 * (fetchData(%cl, "RemortStep") + 1) * $BaseControl[%house])) * (0.5 + (0.5 * $FlagCommand[%house]));
			...
			%rpreward = (0.25 * $BaseControl[%house]) + (0.25 * $FlagCommand[%house]);
