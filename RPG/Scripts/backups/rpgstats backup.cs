...
function DistributeExpForKilling(%damagedClient)
{
	...
	for(%i = 1; %i <= %nameCount; %i++)
	{
		if(%finalDamagedBy[%i] != "")
		{
			...
			{
				%value = 0;
				Client::sendMessage(%listClientId, 0, "You can no longer gain experience. Your body has reached it's peak limit!");
			}
