...
function DoZoneCheck(%w, %d)
{
	...
	deleteObject(%mset);
	if($QuestReload[Chief] > 0)
		$QuestReload[Chief] = $QuestReload[Chief] - 1;
	else if($QuestReload[Chief] == 0)
		$QuestReload[Chief] = "";
	if($QuestReload[DeathKnight] > 0)
		$QuestReload[DeathKnight] = $QuestReload[DeathKnight] - 1;
	else if($QuestReload[DeathKnight] == 0)
		$QuestReload[DeathKnight] = "";
	if($QuestReload[BanishedKing] > 0)
		$QuestReload[BanishedKing] = $QuestReload[BanishedKing] - 1;
	else if($QuestReload[BanishedKing] == 0)
		$QuestReload[BanishedKing] = "";
	if($QuestReload[BattleOx] > 0)
		$QuestReload[BattleOx] = $QuestReload[BattleOx] - 1;
	else if($QuestReload[BattleOx] == 0)
		$QuestReload[BattleOx] = "";
	if($QuestReload[Queen] > 0)
		$QuestReload[Queen] = $QuestReload[Queen] - 1;
	else if($QuestReload[Queen] == 0)
		$QuestReload[Queen] = "";
}
