function Client::onKilled(%clientId, %killerId, %damageType)
{
	...
	if(%killerId != %clientId)
	{
		...
		if(!Player::IsAiControlled(%clientId) && Player::IsAiControlled(%killerId))
			AddBounty(%killerId, (10 * fetchData(%clientId, "LVL")) * (fetchData(%clientId, "RemortStep") + 1));
...
function Player::onKilled(%this)
{
	...
	if(NEWgetClientByName($DuelChallenger) == %clientId || NEWgetClientByName($DuelOpponent) == %clientId)
	{
		%challengerId = NEWgetClientByName($DuelChallenger);
		%opponentId = NEWgetClientByName($DuelOpponent);
		$DuelOn = False;
		if(%clientId == %challengerId)
		{
			$DuelLoser = $DuelChallenger;
			$DuelChallenger = "";
			Client::sendMessage(%challengerId, $MsgRed, "If you want to get your pack back type #duelloser.");
		}
		else
		{
			$DuelLoser = $DuelOpponent;
			$DuelOpponent = "";
			Client::sendMessage(%opponentId, $MsgRed, "If you want to get your pack back type #duelloser.");
		}
	}
...
{
		%norm = 1.000;
		%critskill = $PlayerSkill[%shooterClient, $SkillCriticals]/1250;
		...
		if(!IsDead(%this))
		{
			...
			if(%skilltype >= 1 && !%arenaNull && !%sameTeamNull && %shooterClient != %damagedClient)
			{
				...
				else
				{
					...
					if(%type == $SpellDamageType)
						...
						UseSkill(%damagedClient, $SkillDodging, False, True, %base2 * (3/5));
				}
			...
			if(%value)
			{
				...
				if(%criticalAttack)
					%value *= 2;
				if($SelectedStance[%shooterClient] == "Offensive")
 					%value *= 2;
				if($SelectedStance[%damagedClient] == "Offensive")
					%value *= 2;
				if($SelectedStance[%shooterClient] == "Defensive")
 					%value /= 2;
				if($SelectedStance[%damagedClient] == "Defensive")
					%value /= 2;
				if(fetchData(%damagedClient,"MANA") > 0)
				{
					if(%type == $SpellDamageType && $SelectedStance[%damagedClient] == "MageBane") 
					%value = 0;
					if(%type != $SpellDamageType && $SelectedStance[%damagedClient] == "MageBane")
					%value *= 2;
					if(%type != $SpellDamageType && $SelectedStance[%damagedClient] == "BladeBane")
					%value = 0;
					if(%type == $SpellDamageType && $SelectedStance[%damagedClient] == "BladeBane")
					%value *= 2;
				}
				...
				if(%convValue > 0)
				{
					...
					if(%shooterClient != %damagedClient)
						 if(%criticalAttack)
						Client::sendMessage(%shooterClient, $MsgRed, "You CRITICALLY " @ %saction @ " " @ Client::getName(%damagedClient) @ " for " @ %convValue @ " points of damage!");
					else
				...
				%flash = Player::getDamageFlash(%this) + (%value / (fetchData(%damagedClient, "MaxHP") / 100)) * 4;
				if(%flash > 0.75)
					%flash = 0.75;
