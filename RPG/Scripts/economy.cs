function getBuyCost(%clientId, %item)
{
	...
	%p = round($PlayerSkill[%clientId, $SkillHaggling] / 50) / 100;
	%x = round(%cost * (Cap(%p, 0.0, 0.5) + (0.05 * fetchData(%clientId, "TournyRank"))));
	%cost -= %x;
...
function getSellCost(%clientId, %item)
{
	...
	%p = round($PlayerSkill[%clientId, $SkillHaggling] / 14) / 100;
	%x = round(%cost * (Cap(%p, 0.0, 0.5) + (0.05 * fetchData(%clientId, "TournyRank"))));
	%cost += %x;
